...
const cpp1: TextFillBlankDef = {
  ...
  concept: { en: "Integer division in C++ truncates toward zero; cast to double for floating-point result", ka: "C++-ში მთელი რიცხვის გაყოფა ნულისკენ მისწრაფის; ათწილადისთვის double-ად გარდაქმნა გჭირდება" },
  ...
  task: { en: "Fix the division so the result is a floating-point number.", ka: "გაასწორე გაყოფა ისე, რომ შედეგი ათწილადი იყოს." },
  hints: [
    { en: "What is 7 / 2 in C++ when both operands are int?", ka: "რა არის 7 / 2 C++-ში, როდესაც ორივე წილადის ნაწილი int-ია?" },
    { en: "Integer division discards the remainder — 7/2 = 3, not 3.5.", ka: "მთელი რიცხვის გაყოფა ნაშთს ტოვებს — 7/2 = 3, არა 3.5." },
    { en: "Cast one operand to double: (double)sum / count.", ka: "ერთი ნაწილი double-ად გარდაქმენი: (double)sum / count." },
  ],
  format: "text",
...
console.log(average(10, 4));   // 2.5`,
  ...
  options: [
    ...
    },
    {
      ...
      explanation: { en: "(int)sum still makes the result an integer — no improvement.", ka: "(int)sum შედეგს კვლავ მთელ რიცხვად ტოვებს — შედეგი არ იცვლება." },
    },
    {
      id: "c", value: "(float)", correct: false,
      explanation: { en: "float works but double is preferred in C++ for precision unless memory is constrained.", ka: "float მუშაობს, მაგრამ C++-ში სიზუსტისთვის double-ია სასურველი, თუ მეხსიერება (დამატებითი) არ გვჭირდება." },
    },
  ],
};
...
const cpp2: TextPickFixDef = {
  ...
  story: { en: "A loop that sums an array reads one element past the end, causing undefined behavior.", ka: "ციკლი, რომელიც ითვლის ჯამს, ბოლოს გადასცდება და გაუმართავ ქცევას იწვევს." },
  task: { en: "Fix the loop condition so it stays within bounds.", ka: "გაასწორე ციკლის პირობა, რომ ზღვარს არ გადასცდეს." },
  hints: [
    { en: "If the array has 5 elements, what are the valid indices?", ka: "თუ მასივს 5 ელემენტი აქვს, რა ინდექსებია სწორი?" },
    { en: "The last valid index is size - 1. Your loop goes one step further.", ka: "ბოლო სწორი ინდექსია size - 1. შენი ციკლი ერთი ნაბიჯით სცდება." },
    { en: "Change <= to < in the loop condition.", ka: "ციკლის პირობაში <= შეცვალე <-ით." },
  ],
  format: "text",
  ...
  fixes: [
    {
      ...
      explanation: { en: "i < size stops the loop at index size-1, the last valid position.", ka: "i < size ციკლს size-1 ინდექსზე შეაჩერებს, რაც ბოლო სწორი პოზიციაა." },
    },
    {
      id: "size-minus", correct: false,
      label: { en: "Change size to size-1 in the condition", ka: "პირობაში size → size-1" },
      explanation: { en: "i <= size-1 is equivalent to i < size — that's correct — but keeping <= with size-1 is less idiomatic in C++.", ka: "i <= size-1 i < size-ის ექვივალენტია — სწორია — მაგრამ <= size-1-თან C++-ში ნაკლებ სტანდარტულია." },
    },
    {
      id: "zero", correct: false,
      label: { en: "Start the loop from i = 1 instead", ka: "ციკლი i = 1-დან დავიწყოთ" },
...
const cpp3: TextPickFixDef = {
  ...
  bugType: "wrong-init",
  programmingLanguage: "cpp",
  concept: { en: "Local variables in C++ are not zero-initialized; always initialize before use", ka: "C++-ში ლოკალური ცვლადები ავტომატურად ნულად არ ინიციალიზდება; გამოყენებამდე ყოველთვის ინიციალიზაცია გჭირდება" },
  title: { en: "The Garbage Counter", ka: "უსარგებლო ობიექტების მთვლელი" },
  ...
  task: { en: "Initialize the counter so the function produces reliable results.", ka: "მთვლელს გაუკეთე ინიციალიზაცია, რომ ფუნქცია სანდო შედეგებს გამოსცემდეს." },
  hints: [
...
const cpp9: TextPickFixDef = {
  ...
  hints: [
    ...
    { en: "In the copy constructor, allocate a new array and use memcpy or a loop to copy elements.", ka: "კოპირების კონსტრუქტორში ახალი მასივი გამოყავი და memcpy ან ციკლი გამოიყენე ელემენტების კოპირებისთვის." },
  ],
  format: "text",
  interaction: "pick-fix",
  code: `class Buffer {
...
const cpp11: TextFillBlankDef = {
  ...
  bugType: "wrong-operator",
  programmingLanguage: "cpp",
  concept: { en: "std::endl flushes the output buffer every call; use '\\n' for better performance in tight loops", ka: "std::endl ყოველ გამოძახებაზე ბუფერს ჩამოყრის; მჭიდრო ციკლებში '\\n' გამოიყენე სიჩქარისთვის" },
