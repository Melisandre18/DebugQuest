// POST /api/feedback — emails feedback submissions to the owner.
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  const { context, rating, category, message, puzzleId, route, senderEmail, senderName } = body;
  ...
  const fromLabel = senderName || senderEmail || "Anonymous visitor";
  const replyInfo = senderEmail
    ? `<tr><td style="padding:4px 12px 4px 0"><b>Reply&nbsp;to:</b></td><td><a href="mailto:${senderEmail}">${senderEmail}</a></td></tr>`
    : "";
  const puzzleLine = puzzleId
    ? `<tr><td style="padding:4px 12px 4px 0"><b>Puzzle&nbsp;ID:</b></td><td>${puzzleId}</td></tr>`
    : "";

  const subject = `[DebugQuest] ${category} · ${stars} from ${fromLabel}`;
...
<table style="font-family:sans-serif;border-collapse:collapse;font-size:14px">
  <tr><td style="padding:4px 12px 4px 0"><b>From:</b></td><td>${fromLabel}</td></tr>
  ${replyInfo}
  ...
  ${puzzleLine}
  <tr><td style="padding:4px 12px 4px 0;vertical-align:top"><b>Message:</b></td><td style="white-space:pre-wrap">${String(message).trim()}</td></tr>
...
`;
  ...
  try {
    ...
    const mailOptions: Parameters<typeof transporter.sendMail>[0] = {
      from: `"DebugQuest Feedback" <${gmailUser}>`,
      to: RECIPIENT,
      subject,
      html,
    };
